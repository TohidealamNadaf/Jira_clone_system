...
CREATE TABLE `roadmap_items` (
    ...
    `title` VARCHAR(255) NOT NULL,
    `description` TEXT DEFAULT NULL,
    `type` ENUM('epic', 'milestone', 'phase', 'release') NOT NULL DEFAULT 'epic',
    ...
    KEY `roadmap_items_parent_id_idx` (`parent_id`),
    ...
    CONSTRAINT `roadmap_items_parent_id_fk` FOREIGN KEY (`parent_id`) REFERENCES `roadmap_items` (`id`) ON DELETE CASCADE,
...
CREATE TABLE `roadmap_dependencies` (
    ...
    `dependency_type` ENUM('blocks', 'isblocks', 'requires', 'optional') NOT NULL DEFAULT 'blocks',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `roadmap_dependencies_unique` (`roadmap_item_id`, `depends_on_item_id`),
    CONSTRAINT `roadmap_dependencies_item_fk` FOREIGN KEY (`roadmap_item_id`) REFERENCES `roadmap_items` (`id`) ON DELETE CASCADE,
...
CREATE TABLE `roadmap_item_issues` (
    `roadmap_item_id` INT UNSIGNED NOT NULL,
    `issue_id` INT UNSIGNED NOT NULL,
    PRIMARY KEY (`roadmap_item_id`, `issue_id`),
