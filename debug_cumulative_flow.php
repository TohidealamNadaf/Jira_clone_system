...
if ($board) {
    ...
    $issues = \App\Core\Database::select(
        "SELECT id, `key`, status_id, created_at FROM issues WHERE project_id = ? LIMIT 10",
        [$board['project_id']]
    );
    echo "=== ISSUES ===\n";
    ...
    $history = \App\Core\Database::select(
        "SELECT id, issue_id, field, old_value, new_value, created_at FROM issue_history WHERE field = 'status_id' LIMIT 10"
    );
    echo "=== ISSUE HISTORY ===\n";
