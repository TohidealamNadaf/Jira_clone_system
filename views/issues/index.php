            ...
            <table class="issues-table">
                ...
                <tbody>
                    <?php foreach ($issues['data'] as $issue): ?>
                        <tr class="table-body-row" data-issue-id="<?= $issue['id'] ?>"
                            data-href="<?= url("/issue/{$issue['issue_key']}") ?>">
                                ...
                                <?php if ($issue['reporter_name'] ?? null): ?>
                                    <div class="reporter-cell">
                                        <?php if ($issue['reporter_avatar'] ?? null): ?>
                                            <img src="<?= e(avatar($issue['reporter_avatar'])) ?>" class="avatar-image"
                                                alt="<?= e($issue['reporter_name']) ?>" title="<?= e($issue['reporter_name']) ?>">
                                        <?php else: ?>
                                            <div class="avatar-initial">
                    ...
                    <ul class="pagination-list">
                        <li class="pagination-item <?= $issues['current_page'] <= 1 ? 'disabled' : '' ?>">
                            <a class="pagination-link"
                                href="<?= url("/projects/{$project['key']}/issues?" . http_build_query(array_merge($filters ?? [], ['page' => $issues['current_page'] - 1]))) ?>">
                        ...
                        <?php for ($i = 1; $i <= $issues['total_pages']; $i++): ?>
                            <li class="pagination-item <?= $issues['current_page'] == $i ? 'active' : '' ?>">
                                <a class="pagination-link"
                                    href="<?= url("/projects/{$project['key']}/issues?" . http_build_query(array_merge($filters ?? [], ['page' => $i]))) ?>">
                                    <?= $i ?>
                                </a>
                            </li>
                        <?php endfor; ?>
