            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    <li class="breadcrumb-item"><a
                            href="<?= url('/projects/' . $project['key']) ?>"><?= e($project['key']) ?></a></li>
                    <li class="breadcrumb-item"><a
                            href="<?= url('/projects/' . $project['key'] . '/boards') ?>">Boards</a></li>
        ...
        <div class="d-flex gap-2">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu dropdown-menu-end" style="width: 250px;">
                    <li class="px-3 py-2">
                        <input type="text" class="form-control form-control-sm" id="quickFilter"
            ...
            <?php if (can('manage-boards', $project['id'])): ?>
                <a href="<?= url('/projects/' . $project['key'] . '/boards/' . $board['id'] . '/settings') ?>"
            ...
            <div class="board-column" data-column-id="<?= $column['id'] ?>">
                <div class="column-header" style="border-color: <?= e($column['color'] ?? '#6c757d') ?>">
                    ...
                    <?php if (!empty($column['wip_limit'])): ?>
                        <span
                            class="badge <?= count($column['issues'] ?? []) > $column['wip_limit'] ? 'bg-danger' : 'bg-light text-dark' ?>">
                        ...
                        <div class="issue-card" draggable="true" data-issue-id="<?= $issue['id'] ?>"
                            ...
                            <div class="d-flex align-items-center mb-1">
                                ...
                                <a href="<?= url('/issue/' . $issue['issue_key']) ?>"
                                    class="issue-key text-primary text-decoration-none">
                                    <?= e($issue['issue_key']) ?>
                                </a>
                                <span class="ms-auto badge"
                                    style="background-color: <?= e($issue['priority_color']) ?>; font-size: 0.65rem;">
                            ...
                            <div class="issue-meta text-muted">
                                ...
                                <?php if ($issue['assignee_avatar'] ?? null): ?>
                                    <img src="<?= e(avatar($issue['assignee_avatar'])) ?>" class="rounded-circle" width="24"
                                        height="24" title="<?= e($issue['assignee_name']) ?>">
    ...
    <div class="modal fade" id="completeSprintModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form
                    action="<?= url('/projects/' . $project['key'] . '/sprints/' . ($activeSprint['id'] ?? '') . '/complete') ?>"
                    method="POST">
                    <?= csrf_field() ?>
