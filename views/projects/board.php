    ...
    <div class="board-container">
        ...
        <div class="kanban-board">
            <?php foreach ($statuses as $status): ?>
                ...
                ?>
                <div class="kanban-column" data-status-id="<?= e($status['id']) ?>"
                    ...
                    <div class="column-cards board-column" data-status-id="<?= e($status['id']) ?>">
                        ...
                        <?php else: ?>
                            ...
                            <?php foreach ($statusIssues as $issue): ?>
                                <div class="issue-card" draggable="true" data-issue-id="<?= e($issue['id']) ?>"
                                    data-issue-key="<?= e($issue['issue_key']) ?>" data-status-id="<?= e($status['id']) ?>"
                                    data-priority="<?= e($issue['priority_name'] ?? 'Medium') ?>"
                                    data-assignee="<?= e($issue['assignee_name'] ?? 'Unassigned') ?>" role="button" tabindex="0">
                                                ...
                                                <?php if ($issue['assignee_name']): ?>
                                                    <?php if (!empty($issue['assignee_avatar']) && is_file($_SERVER['DOCUMENT_ROOT'] . parse_url($issue['assignee_avatar'], PHP_URL_PATH))): ?>
                                                        <img src="<?= e(avatar($issue['assignee_avatar'])) ?>"
                                                            alt="<?= e($issue['assignee_name']) ?>" class="avatar-sm"
