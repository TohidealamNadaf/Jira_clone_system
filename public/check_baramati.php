...
$sql = "SELECT * FROM projects WHERE name LIKE '%baramati%' OR name LIKE '%Baramati%' OR `key` LIKE '%BARA%'";
$stmt = $pdo->query($sql);
...
if (empty($projects)) {
    ...
    foreach ($projects as $project) {
        echo "Project Name: {$project['name']} (Key: {$project['key']})\n";
        echo "ID: {$project['id']}\n";
        echo "Created: {$project['created_at']}\n";
        echo "Archived: " . ($project['is_archived'] ? 'YES' : 'NO') . "\n";
        echo "Issue Count: {$project['issue_count']}\n\n";
    }
}
...
if (empty($allProjects)) {
    ...
    foreach ($allProjects as $proj) {
        ...
        echo "{$proj['key']}: {$proj['name']} $status (Issues: {$proj['issue_count']}, Created: {$proj['created_at']})\n";
    }
}
