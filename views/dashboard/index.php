...
<div class="dashboard-wrapper">
  ...
  <div class="dashboard-container">
    ...
    <div class="dashboard-stats-grid">
      ...
      <a href="<?= url('/search?assignee=currentUser()&due=overdue') ?>"
        class="stat-card <?= ($stats['overdue'] ?? 0) > 0 ? 'danger' : '' ?>">
        <div class="stat-icon">⚠️</div>
        <div>
      ...
      <div>
        ...
        <div class="card">
          ...
          <div class="tabs-wrapper" role="tablist">
            <button class="tab-button active" id="assigned-tab" data-bs-toggle="tab" data-bs-target="#assigned-panel"
              role="tab">
              Assigned to Me <span class="tab-badge"><?= count($assignedIssues ?? []) ?></span>
            </button>
            <button class="tab-button" id="reported-tab" data-bs-toggle="tab" data-bs-target="#reported-panel"
              role="tab">
              ...
              <div class="tab-pane fade show active" id="assigned-panel" role="tabpanel">
                <?php if (empty($assignedIssues)): ?>
                  <div class="empty-state">
                    ...
                    <button class="dashboard-btn dashboard-btn-primary" data-bs-toggle="modal"
                  ...
                  <div class="issue-list">
                    <?php foreach ($assignedIssues as $issue): ?>
                      ...
                      <a href="<?= url('/issue/' . $issue['issue_key']) ?>" class="issue-row"
                        data-priority="<?= e($prioritySlug) ?>" data-due="<?= e($dueBucket) ?>"
                        data-updated-today="<?= $updatedToday ? 'true' : 'false' ?>">
                        <div class="issue-type-icon"
                          style="background-color: <?= e($issue['issue_type_color'] ?? 'var(--jira-blue)') ?>">
                        ...
                        <span class="issue-priority"
                          style="background-color: <?= e($issue['priority_color'] ?? '#626F86') ?>">
                          <?= e(substr($issue['priority_name'] ?? 'None', 0, 1)) ?>
                        </span>
                        <span class="issue-status"
                          style="background-color: <?= e($issue['status_color'] ?? 'var(--jira-blue)') ?>; color: white !important;">
