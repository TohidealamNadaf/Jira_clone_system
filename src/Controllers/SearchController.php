...
class SearchController extends Controller
{
    public function index(Request $request): string
    {
        ...
        $filters = [
            ...
            'type' => $this->ensureArray($request->input('type', [])),
            'status' => $this->ensureArray($request->input('status', [])),
            'priority' => $this->ensureArray($request->input('priority', [])),
    ...
    private function performSearchWithFilters(string $query, array $filters, string $sort, int $page, int $perPage): array
    {
        ...
        // Type filter - convert names to IDs if needed
        if (!empty($filters['type'])) {
            $typeValues = $this->convertToIds($filters['type'], 'issue_types', 'name');
            if (!empty($typeValues)) {
                $placeholders = implode(',', array_fill(0, count($typeValues), '?'));
                $conditions[] = "i.issue_type_id IN ({$placeholders})";
                $params = array_merge($params, $typeValues);
            }
        }

        // Status filter - convert names to IDs if needed
        if (!empty($filters['status'])) {
            $statusValues = $this->convertToIds($filters['status'], 'statuses', 'name');
        ...
        // Priority filter - convert names to IDs if needed
        if (!empty($filters['priority'])) {
            $priorityValues = $this->convertToIds($filters['priority'], 'issue_priorities', 'name');
            if (!empty($priorityValues)) {
                $placeholders = implode(',', array_fill(0, count($priorityValues), '?'));
                $conditions[] = "i.priority_id IN ({$placeholders})";
                $params = array_merge($params, $priorityValues);
            }
        }
        ...
        // Exclude done status filter (auto-applied when viewing assignee=currentUser without explicit status filter)
        if ($filters['exclude_done']) {
            $conditions[] = "s.category != 'done'";
        }
     ...
     * Converts string or null to array format
     */
    ...
    /**
     * Convert filter values to their corresponding IDs
